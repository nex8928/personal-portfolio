...
const SkillCard: React.FC<SkillCardProps> = ({ category: cat, index }) => {
  ...
  return (
    <div ref={ref} className={`glass-panel card-glow animate-in stagger-${(index % 4) + 1} ${isVisible ? 'visible' : ''}`}
      style={{ gridColumn: `span ${cat.colSpan}`, padding: '32px', position: 'relative', overflow: 'hidden', transition: 'border-color 0.3s' }}
      onMouseEnter={(e) => e.currentTarget.style.borderColor = 'var(--outline)'}
      onMouseLeave={(e) => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'}>
      <div style={{ position: 'absolute', top: 0, right: 0, padding: '16px' }} className="font-mono">
        <span style={{ color: 'var(--outline-variant)', transition: 'color 0.3s' }}>{cat.moduleId}</span>
      </div>
  ...
  return (
    <section ref={ref} className={`animate-in bento-grid ${isVisible ? 'visible' : ''}`} style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 'var(--gutter)' }}>
      ...
      <div style={{ gridColumn: 'span 7', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <div className="glass-panel" style={{ padding: '32px', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flex: 1 }}>
          <div style={{ position: 'absolute', top: 0, right: 0, padding: '16px' }} className="font-mono"><span style={{ color: 'var(--outline-variant)' }}>{education.status}</span></div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '16px' }}>
            ...
            <p className="font-body" style={{ color: 'var(--on-surface-variant)', maxWidth: '480px', marginTop: '8px' }}>{education.description}</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '48px', borderTop: '1px solid rgba(68,71,72,0.5)', paddingTop: '24px' }}>
            <div>
      ...
      <div style={{ gridColumn: 'span 5', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {milestones.map((m) => (
            <div key={m.title} className="glass-panel card-glow" style={{ padding: '24px', display: 'flex', alignItems: 'flex-start', gap: '16px', transition: 'all 0.3s', cursor: 'default' }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--outline)'; e.currentTarget.style.backgroundColor = 'rgba(32,32,31,0.6)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; e.currentTarget.style.backgroundColor = 'rgba(32,32,31,0.4)'; }}>
              <div style={{ backgroundColor: 'var(--surface-container-high)', padding: '12px', border: '1px solid var(--outline-variant)' }}>
                <span className="material-symbols-outlined" style={{ color: m.accentColor === 'lime' ? 'var(--tertiary)' : 'var(--secondary-container)' }}>{m.icon}</span>
              </div>
              <div>
                ...
                <div style={{ marginTop: '12px' }}>
                  <span className="font-mono" style={{ color: m.accentColor === 'lime' ? 'var(--tertiary)' : 'var(--secondary-container)', border: `1px solid ${m.accentColor === 'lime' ? 'rgba(171,214,0,0.3)' : 'rgba(0,241,254,0.3)'}`, padding: '2px 8px', fontSize: '10px' }}>{m.platform}</span>
                </div>
              </div>
            </div>
          ))}
